<script lang="ts">
	...
	function toggleShowApiKeyInput() {
		showApiKeyInput = !showApiKeyInput
	}

	function resetSubmittedApiKey() {
		submittedApiKey = false
	}

	$: submittedApiKey && checkApiKey()
...
{#key reloadFolders}
	<div class="{mode}">
		<div class="min-h-full h-screen bg-white dark:bg-offblack text-black dark:text-offwhite focus:outline-columbia overflow-y-auto">		
			<!-- bar at top -->
			<div class=" mb-10 w-full">
				...
				<!-- dark mode button -->
				<div class="mt-5 max-w-2xl flex flex-row justify-end mx-auto">
					<div class="">
						{#if mode == "dark"}
							<svg 
					...
					<div class="ml-5 mr-2">
						{#if showApiKeyInput}
							<!-- key-filled -->
							<svg 
								class="flex-none h-8 w-8 cursor-pointer fill-columbia-dark dark:fill-columbia pt-2" 
								on:click={toggleShowApiKeyInput} on:keydown={toggleShowApiKeyInput} 
						...
						{:else}
							<!-- key-open -->
							<svg 
								class="flex-none h-8 w-7 cursor-pointer stroke-columbia-dark dark:stroke-columbia pt-2" 
								on:click={toggleShowApiKeyInput} on:keydown={toggleShowApiKeyInput} 
