...
{#if entry_data}
	<div class="w-full"> 
		<div class="h-8 flex justify-between">
			...
			<div class="">
				<span 
					class="font-bold cursor-pointer dark:invert text-blacktext -ml-5 float-left {entry_data.is_expanded ? "h-8" : "h-7"} {settingsTrayOpen ? "text-columbia dark:text-inverted-columbia font-extrabold" : ""}" on:click={toggleExpanded} on:keydown={toggleExpanded}
